...
let conf = {
	...
	output: {
		path: path.resolve(__dirname, './public/javascripts'), //путь к папке в которую кладем файл main.js, путь делается абсолютным через модуль path
		filename: "bundle.js",
		publicPath: "public/javascripts" //подставляем папку dist и теперь можем не пересобирать, изменения будут отображаться в браузере на лету
	},
	devServer: {
